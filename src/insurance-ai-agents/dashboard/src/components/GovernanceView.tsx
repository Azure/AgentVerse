...
function Pill({ ok, children }: { ok: boolean; children: React.ReactNode }) {
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-medium ${ok ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-800'
        }`}
    >
...
export function GovernanceView() {
  ...
  if (!data) {
    return <div className="p-6 text-gray-500">Loading governance status…</div>;
  }
    ...
    <div className="space-y-6 p-6">
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-bold text-gray-900">AI Control Center</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-gray-600">
              Every AI call passes through an AI Gateway with managed identity, content safety and token metrics.
              Every prompt change goes through an automatic evaluation pipeline before reaching production.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="mb-2 flex items-center justify-between">
            ...
            <Pill ok={apimActive}>{apimActive ? 'Active' : 'Direct mode'}</Pill>
          </div>
          <div className="text-2xl font-bold text-gray-900">
            {apimActive ? `${data.apim.policies.filter((policy) => policy.active).length} policies` : 'Bypass for dev'}
          </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="mb-2 flex items-center justify-between">
            ...
            {passRate !== null ? <Pill ok={passRate >= 80}>{passRate}% pass</Pill> : <span className="text-xs text-gray-500">no runs</span>}
          </div>
          ...
          <div className="mt-1 text-xs text-gray-500">
            {data.evals.latest
              ? `Last run: ${new Date(data.evals.latest.timestamp).toLocaleString()}`
              : 'No evals have been run yet'}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-semibold text-gray-900">Active policies in the AI Gateway</h2>
          </div>
          ...
          <div className="mt-4 border-t border-gray-200 pt-4 text-xs text-gray-500">
            Defined in <code className="rounded bg-gray-100 px-1 py-0.5">infra/apim-policy.xml</code> and applied by Bicep when deploying APIM StandardV2.
          </div>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-semibold text-gray-900">Evaluation pipeline</h2>
          </div>
          {data.evals.latest ? (
            ...
            <div className="text-sm text-gray-500">
              The golden dataset has not been run yet. Run:
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-900">Code ownership (CODEOWNERS)</h2>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 text-left text-xs uppercase text-gray-500">
                ...
                <th className="py-2">Responsible teams</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className="mt-3 text-xs text-gray-500">
          GitHub automatically blocks PRs that touch these paths without approval from the corresponding teams.
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-900">Process checks & auditable evidence</h2>
        </div>
        <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
          {Object.entries(data.checks).map(([key, value]) => (
            <div key={key} className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
              ...
              <Pill ok={value}>{value ? 'present' : 'missing'}</Pill>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-2 text-center text-xs text-gray-500">
        ...
        commit {data.git_commit} · pipeline v{data.pipeline_version} · refreshed {new Date(data.deployed_at).toLocaleTimeString()}
      </div>
    </div>
  );
}
...
function prettifyCheck(key: string): string {
  const map: Record<string, string> = {
    ...
    deploy_workflow: 'Per-agent deployment workflow',
    eval_workflow: 'Eval-Gate workflow on PRs',
    infra_as_code: 'Infrastructure as code (Bicep)',
    apim_policy_xml: 'AI Gateway XML policy',
  };
  return map[key] || key;
}
